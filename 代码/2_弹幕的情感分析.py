...
def sentiment_analyse(v_cmt_list):
	...
	plt.title('弹幕_情感分布占比图')
	plt.savefig('弹幕_情感分布占比图.png')  # 保存图片
	# 把情感分析结果保存到excel文件
	df.to_excel('弹幕_情感评分结果.xlsx', index=None)
	print('情感分析结果已生成：弹幕_情感评分结果.xlsx')
...
def make_wordcloud(v_str, v_stopwords, v_outfile):
	...
	try:
		...
		backgroud_Image = np.array(Image.open('心型.jpg'))  # 读取背景图片
		wc = WordCloud(
